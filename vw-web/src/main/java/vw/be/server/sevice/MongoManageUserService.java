	...
	public Future<Collection<UserDTO>> getAllUsers() {
		...
		mongoClient.find(COLLECTION, new JsonObject(), findAllResultHandler -> {
			if (findAllResultHandler.succeeded()) {
				Collection<UserDTO> users = findAllResultHandler.result().stream()
						.map(user -> new UserDTO(user.getValue("_id").toString(), user.getLong("version"),
								user.getString("firstName"), user.getString("surname"), user.getString("lastName")))
