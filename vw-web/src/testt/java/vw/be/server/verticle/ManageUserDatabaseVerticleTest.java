...
/**
 * This is our JUnit test for our MONGO/Mock persistence service for user management. The test uses vertx-unit, so we
...
public class ManageUserDatabaseVerticleTest {
    ...
    public void getUserById(TestContext context) {
        ...
        vertx.eventBus().send(IManageUserService.MANAGE_USER_DB_QUEUE,
                              userToCreate,
                              new DeliveryOptions().addHeader(PERSISTENCE_ACTION, String.valueOf(CREATE)),
                              replyOfCreate -> {
                                  if (replyOfCreate.succeeded()) {
                                      ...
                                      context.assertTrue(
                                              String.valueOf(CREATED)
                                                    .contentEquals(headers.get(PERSISTENCE_RESPONSE_CODE)));
                                      ...
                                      vertx.eventBus().send(IManageUserService.MANAGE_USER_DB_QUEUE, getByIdRequest,
                                                            new DeliveryOptions().addHeader(PERSISTENCE_ACTION,
                                                                                            String.valueOf(GET_BY_ID)),
                                                            replyOfGet -> {
                                                                if (replyOfGet.succeeded()) {
                                                                    ...
                                                                    context.assertTrue(user.toString()
        ...
        vertx.eventBus().send(IManageUserService.MANAGE_USER_DB_QUEUE,
                              ...
                              replyOfCreate -> {
                                  if (replyOfCreate.succeeded()) {
                                      ...
                                      MultiMap headers = replyOfCreate.result().headers();
                                      context.assertTrue(headers.contains(PERSISTENCE_RESPONSE_CODE));
                                      context.assertEquals(String.valueOf(CREATED),
                                                           headers.get(PERSISTENCE_RESPONSE_CODE));
                                      ...
                                      vertx.eventBus().send(IManageUserService.MANAGE_USER_DB_QUEUE, userToEdit,
                                                            ...
                                                            replyOfMerge -> {
                                                                if (replyOfMerge.succeeded()) {
                                                                    MultiMap mergeHeaders = replyOfMerge.result()
                                                                                                        .headers();
                                                                    context.assertTrue(mergeHeaders.contains(
                                                                            PERSISTENCE_RESPONSE_CODE));
    ...
    private void sendRequest(TestContext context, JsonObject request,
        ...
        vertx.eventBus().send(IManageUserService.MANAGE_USER_DB_QUEUE,
                              request,
                              new DeliveryOptions().addHeader(PERSISTENCE_ACTION, String.valueOf(DELETE_BY_ID)),
                              responce -> {
                                  if (responce.succeeded()) {
                                      MultiMap headers = responce.result().headers();
                                      context.assertTrue(headers.contains(PERSISTENCE_RESPONSE_CODE));
                                      context.assertEquals(String.valueOf(expectedResult),
                                                           headers.get(PERSISTENCE_RESPONSE_CODE));
                                  } else {
